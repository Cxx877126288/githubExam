...
public class Spell extends AppCompatActivity {
	...
	private List<Integer> hasAppeared = new ArrayList<>();
	private List<Integer> wordsID;
	private WordService ws = new WordServiceImpl();
	private List<Integer> views;
	private Word word;
	private List<Integer> widgetID;
	...
	private void loadpage() {
		...
		if (hasAppeared.size() < wordsID.size()) {
			int index = random.nextInt( wordsID.size() );
			while ((haveAppeared( index ))) {
				index = random.nextInt( wordsID.size() );
			}
		...
		if (word != null) {
			...
			for (int i = 0; i < chars.length; i++) {
				if (ExpandUtil.NumberExpand( random.nextInt( 200000 ) ) == 0) {
					//为0就生成空
